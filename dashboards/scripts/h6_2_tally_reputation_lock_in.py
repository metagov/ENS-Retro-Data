...
Activity vs. delegated voting power scatter plot (Tally on-chain, past 12 months)
"""
...
def _load_data() -> pd.DataFrame:
    ...
    df = con.execute("""
        ...
        delegate_tally_votes_12m AS (
            SELECT
                tv.voter AS address,
                COUNT(DISTINCT tv.proposal_id) AS proposals_voted
            FROM main_silver.clean_tally_votes tv
            JOIN main_silver.clean_tally_proposals tp
                ON tv.proposal_id = tp.proposal_id
            WHERE tp.start_date >= CURRENT_DATE - INTERVAL '12 months'
            GROUP BY tv.voter
        ),
        top_delegates AS (
        ...
        LEFT JOIN delegate_tally_votes_12m dv ON d.address = dv.address
        CROSS JOIN tally_proposals_12m tp
...
def _build_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        xaxis=dict(
            title=dict(text="Tally vote participation rate — past 12 months", font=dict(size=13, color="#4A5568")),
            range=[-2, 102],
            tickformat=".0f",
            ticksuffix="%",
...
def render_tally_activity_vs_vp() -> None:
    ...
    st.caption(
        ...
        f"(<{PARTICIPATION_THRESHOLD}% Tally participation). "
        f"Dot size proportional to delegated VP. "
        f"Participation threshold: ≥{PARTICIPATION_THRESHOLD}% of Tally on-chain proposals."
    )
    st.caption(
        "**Sources:** ENS Tally on-chain governance data. Participation rate computed over "
        "Tally proposals with start date in the past 12 months. "
