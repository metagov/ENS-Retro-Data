...
def _classify(row: pd.Series) -> str:
    ...
    elif row["vp_k"] >= VP_THRESHOLD:
        return "Lock-in zone (high VP, <50% participation)"
    else:
        return "Low VP + low activity"


_COLOR_MAP = {
    "Active (>50% participation)":             "#5B6AD6",
    "Lock-in zone (high VP, <50% participation)": "#E07B54",
    "Low VP + low activity":                   "#BBBBBB",
}
...
def _build_chart(df: pd.DataFrame) -> go.Figure:
    ...
    fig.add_shape(
        ...
        y0=VP_THRESHOLD, y1=max_vp * 1.15,
    ...
    # Horizontal threshold line at 100K VP
    fig.add_hline(
        y=VP_THRESHOLD,
        line=dict(color="#888888", dash="dash", width=1.5),
    )
