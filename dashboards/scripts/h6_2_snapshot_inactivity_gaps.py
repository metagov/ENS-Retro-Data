...
24-month window Jan 2024 – Dec 2025. Snapshot only.
"""
...
def _load_data() -> pd.DataFrame:
    ...
    df = con.execute(f"""
        ...
        delegate_votes_by_month AS (
            SELECT
                sv.voter                                        AS address,
                DATE_TRUNC('month', sp.start_date)::DATE        AS month,
                COUNT(DISTINCT sv.proposal_id)                  AS votes_cast
            FROM main_silver.clean_snapshot_votes sv
            JOIN main_silver.clean_snapshot_proposals sp
                ON sv.proposal_id = sp.proposal_id
            WHERE sp.start_date >= '{START_DATE}'
              AND sp.start_date <  '{END_DATE}'
            GROUP BY sv.voter, DATE_TRUNC('month', sp.start_date)::DATE
        ),
        top_delegates AS (
...
def render_snapshot_inactivity_gaps() -> None:
    st.markdown(
        ...
        "24-month view, Jan 2024 – Dec 2025. Snapshot only.</p>",
    ...
    if df.empty:
        st.warning("No Snapshot data found for the Jan 2024 – Dec 2025 window.")
        return

    fig = _build_chart(df)
    render_chart(fig, key="dl_snapshot_inactivity", filename="snapshot_inactivity_gaps", width='stretch')
    ...
    st.caption(
        ...
        f"(proposals existed but the delegate did not vote). "
        f"Months with no proposals shown in light gray."
    )
    st.caption(
        "**Sources:** ENS Snapshot governance data."
    )
